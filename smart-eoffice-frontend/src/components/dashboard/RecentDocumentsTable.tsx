...
import { RecentDocumentItem } from '@/services/dashboardService';
...
export const RecentDocumentsTable: React.FC<RecentDocumentsTableProps> = ({
  ...
  emptyDescription = 'There are no documents to display in this list.',
  className = '',
}) => {
  return (
              ...
              <Table.Tr key={doc.id}>
                <Table.Td>
                  <div>
                    ...
                    <span className="text-xs text-[#8A8983]">ID: {doc.documentNumber}</span>
                  </div>
                </Table.Td>
                <Table.Td>
