...
export const Archives: React.FC = () => {
  ...
  const fetchArchives = useCallback(async () => {
    ...
    try {
      ...
      const res = await documentService.getArchivedDocuments(params);
      setResponse(res);
    } catch (err: any) {
      setError(err.message || 'Failed to load archived documents.');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownload = async (doc: DocumentItem) => {
    try {
      ...
      addToast({
        ...
        message: err.message || 'Unable to download archived document.',
      });
    }
  };
  ...
  const handleConfirmRestore = async () => {
    ...
    setIsRestoring(true);
    try {
      await documentService.restoreDocument(restoreTarget.id);
      addToast({
        type: 'success',
        title: 'Document Restored',
        message: `"${restoreTarget.title}" has been restored to active document storage.`,
      });
      ...
      addToast({
        ...
        message: err.message || 'Could not restore document from archive.',
      });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-[#D8D7D1]/60">
        <div>
          ...
          <p className="text-xs md:text-sm text-[#6B6A64] mt-1 font-normal">
            Browse and manage official archived documents and historical records across SITA units.
          </p>
        </div>
      ...
      <div className="text-xs font-semibold text-[#6B6A64]">
        Showing {startCount}–{endCount} of {totalDocuments} archived documents
      </div>
        ...
        <EmptyState
          ...
          description={
            search || category !== 'ALL' || department !== 'ALL'
              ? 'No archived documents match your selected filters. Try clearing filters.'
              : 'There are currently no archived records stored in the system vault.'
          }
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to restore "${restoreTarget?.title}"? It will be moved from the archive back to active document storage.`}
