...
const MOCK_LETTERS: LetterItem[] = [
  {
    ...
    subject: 'Request for Digital Transformation Progress Report & Budget Alignment',
    description: 'Official correspondence from the Ministry of Finance requesting SITA digital transformation milestones, Q4 expenditure, and budget realignment proposal.',
    letterType: 'REQUEST',
    category: 'Finance / Budget',
    department_name: 'ICT Governance',
    currentDepartment: 'ICT Governance',
    ...
    is_new: false,
    assignment: {
      assignedDepartment: 'ICT Governance',
      assignedUser: 'Endrias Eshetu',
      assignedUserId: 'user-3',
      assignedBy: 'Tigist Haile (ICT Manager)',
      assignmentDate: 'Aug 21, 2026',
      dueDate: 'Sep 02, 2026',
      instructions: 'Prepare detailed status report on SITA Smart E-Office migration and budget deployment for Q4.',
      priority: 'HIGH',
    ...
    relatedLetters: [
      {
        ...
        subject: 'Digital Transformation Progress Report & Q4 Expenditure Breakdown',
        direction: 'OUTGOING',
    ...
    movements: [
      ...
      },
      {
        ...
        actorRole: 'ADMIN',
        action: 'Routed to ICT Governance Department',
        timestamp: 'Aug 20, 2026 02:00 PM',
        previousStatus: 'RECEIVED',
        newStatus: 'RECEIVED',
        department: 'ICT Governance',
        comment: 'Assigned to ICT Directorate for technical preparation.',
      },
      {
        id: 'mov-4',
  ...
  },
  {
    ...
    subject: 'Official Response – SITA Digital Transformation Progress & Q4 Allocation Proposal',
  ...
  },
  {
    ...
    subject: 'Internal Memorandum – IT Equipment & Server Procurement Request',
    description: 'Internal communication from ICT Directorate to Procurement Directorate for datacenter hardware upgrades.',
    letterType: 'MEMORANDUM',
    category: 'Procurement / Supplies',
    department_name: 'Procurement & Finance',
    targetDepartment: 'Procurement & Finance',
    originatingDepartment: 'ICT Governance',
    currentDepartment: 'Procurement & Finance',
    currentResponsibleUser: 'Procurement Manager',
    currentTask: 'Assign Procurement Specialist',
    sender: 'ICT Director (Tigist Haile)',
    senderOrganization: 'SITA - ICT Governance Directorate',
    recipient: 'Procurement Department Head',
    ...
    dueDate: 'Sep 10, 2026',
    assignment: {
      assignedDepartment: 'Procurement & Finance',
      taskStatus: 'PENDING_ACTION',
      dueDate: 'Sep 10, 2026',
      instructions: 'Process tendering document for 4 rack servers.',
    },
    movements: [
      {
        id: 'mov-3-1',
      ...
      },
      {
        ...
        actorName: 'Main Administrator',
        actorRole: 'ADMIN',
        action: 'Registered & Routed to Procurement Dept (Ref: INT/2026/00317)',
  ...
  },
  {
    ...
    description: 'Official invitation from African Union Commission for SITA delegation to attend the Regional Innovation Summit.',
  ...
  },
  {
    ...
    description: 'Formal notification from Office of Auditor General regarding upcoming audit schedule for ICT infrastructure and procurement.',
...
export const letterService = {
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        ...
        if (params?.taskStatus && params.taskStatus !== 'ALL') {
          filtered = filtered.filter((l) => l.assignment?.taskStatus === params.taskStatus);
        }
  ...
  async createLetter(
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        const direction = (formData.get('direction') as LetterDirection) || 'INCOMING';
        const subject = (formData.get('subject') as string) || 'Untitled Official Letter';
        const letterType = (formData.get('letterType') as LetterType) || direction;
        ...
        const priority = (formData.get('priority') as LetterPriority) || 'NORMAL';
        const description = (formData.get('description') as string) || '';
        const sender = (formData.get('sender') as string) || '';
        const senderOrganization = (formData.get('senderOrganization') as string) || '';
        const recipient = (formData.get('recipient') as string) || '';
        const recipientOrganization = (formData.get('recipientOrganization') as string) || '';
        const externalReferenceNumber = (formData.get('externalReferenceNumber') as string) || '';
        ...
        const newLetter: LetterItem = {
          ...
          category,
          department_name,
          currentDepartment: direction === 'INCOMING' ? 'Main Administrator Office' : department_name,
          currentResponsibleUser: direction === 'INCOMING' ? 'Main Administrator' : 'Department Manager',
          ...
          confidentialityLevel,
          priority,
          dateReceived: direction === 'INCOMING' ? now.toLocaleDateString() : undefined,
          dateSent: direction === 'OUTGOING' ? now.toLocaleDateString() : undefined,
          ...
          movements: [
            {
              ...
              action: direction === 'INCOMING' ? 'Registered Incoming Letter' : 'Created Draft Letter',
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        ...
        if (target) {
          ...
          return { message: `Letter routed to ${departmentName} successfully.`, letter: target };
        }
      }
      throw error;
    }
  ...
  async assignToOfficer(
    id: string,
    payload: { officerName: string; dueDate?: string; instructions?: string; priority?: LetterPriority }
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        ...
        if (target) {
          ...
          return { message: `Assigned to ${payload.officerName}.`, letter: target };
        }
      }
      throw error;
    }
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        ...
        if (target) {
          ...
          return { message: `Registered with reference ${officialRef}.`, letter: target };
        }
      }
      throw error;
    }
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK' || !error.response) {
        ...
        if (target) {
          ...
          target.dateSent = dispatchInfo.dispatchDate || new Date().toLocaleDateString();
