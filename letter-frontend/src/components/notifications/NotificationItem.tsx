...
const typeIcons: Partial<
  Record<NotificationType, { icon: React.ReactNode; color: string }>
> = {
  LETTER_REGISTERED: {
...
export const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  const meta = typeIcons[notification.type] ??
    typeIcons.LETTER_REGISTERED ?? {
      color: "bg-[#D8D7D1]/60 text-[#6B6A64]",
      icon: null,
    };
  ...
  return (
    <div
      ...
      className={`p-3 rounded-xl flex items-start space-x-3 transition-colors cursor-pointer ${
        notification.isRead
          ? "hover:bg-[#ECEAE3]"
          : "bg-[#526A55]/08 hover:bg-[#526A55]/12 font-medium"
      }`}
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-[#292A27] leading-snug">
